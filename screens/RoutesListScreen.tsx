...
import { getEta, getRoutes, getShuttles, getShuttlesWithEta } from '../utils/api';
...
function getShuttleForRoute(shuttles: Shuttle[], routeId: string): Shuttle | undefined {
...
export default function RoutesListScreen({ navigation }: RoutesListScreenProps): React.JSX.Element {
  ...
  try {
    const [routesData, shuttlesData] = await Promise.all([
      getRoutes(),
      getShuttlesWithEta(),
    ]);
...
useEffect(() => {
  ...
  const renderRouteCard = ({ item }: { item: Route }) => {
    const shuttle = getShuttleForRoute(shuttles, item.routeId);
